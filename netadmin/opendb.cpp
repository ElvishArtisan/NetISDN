...
//
//      $Id: opendb.cpp,v 1.2 2008/11/03 17:17:00 cvs Exp $
//
	    ...
	    QString driver)
{
  ...
  db->setHostName(host);
  if(!db->open()) {
    msg=QT_TR_NOOP("Unable to access the NetISDN Database!\n\
...
and we will try to get this straightened out.");
	...
	if(!q->isActive()) {   // Can't create DB.
	  ...
	  QMessageBox::warning(NULL,QT_TR_NOOP("Can't Create DB"),
			 QT_TR_NOOP("Unable to create a NetISDN Database!"));
	  db->removeDatabase("mysql");
	  return false;
	}
	delete q;
	...
	if(!db->open()) {   // Can't open new database
	  QMessageBox::warning(NULL,QT_TR_NOOP("Can't Connect"),
		 QT_TR_NOOP("Unable to connect to new NetISDN Database!"));
	  db->removeDatabase(dbname);
	  return false;
	}
	if(!CreateDb(login,pwd)) {   // Can't create tables.
	  QMessageBox::warning(NULL,QT_TR_NOOP("Can't Create"),
			       QT_TR_NOOP("Unable to create NetISDN Database!"));
	  db->removeDatabase(dbname);
	  return false;
	}
	db->close();
	...
	if(!db->open()) {
	  QMessageBox::warning(NULL,QT_TR_NOOP("Can't Connect"),
			       QT_TR_NOOP("Unable to connect to NetISDN Database!"));
	  db->removeDatabase(dbname);
	  return false;
	}	  
	if(!InitDb(login,pwd)) {  // Can't initialize tables.
	  QMessageBox::warning(NULL,QT_TR_NOOP("Can't Initialize"),
			       QT_TR_NOOP("Unable to initialize NetISDN Database!"));
	  db->removeDatabase(dbname);
	  return false;
	}
	QMessageBox::information(NULL,QT_TR_NOOP("Created Database"),
			     QT_TR_NOOP("New NetISDN Database Created!"));
	return true;
      }
    }
    return false;
  }
  if((db_ver=CheckVersion())<NET_VERSION_DATABASE) {
    if(db_ver==0) {    // Pre-historic database version!
      msg=QT_TR_NOOP("The NetISDN Database is too old to be upgraded,\n\
...
with administrative privledges, otherwise hit cancel.");
	...
	QMessageBox::warning(NULL,QT_TR_NOOP("Can't Create DB"),
		  QT_TR_NOOP("Unable to authorize a NetISDN Database!"));
	db->removeDatabase("mysql");
	return false;
      }
      db->close();   // Relinquish admin perms
	...
	if(QMessageBox::warning(NULL,QT_TR_NOOP("Update Needed"),
				QT_TR_NOOP("The NetISDN Database needs to be updated.\n\
...
on this machine for a few seconds.  Continue?"),
	...
	QMessageBox::warning(NULL,QT_TR_NOOP("Can't Update"),
			     QT_TR_NOOP("Unable to update NetISDN Database!"));
	db->removeDatabase(dbname);
	return false;
    }
    str=QString(
	QT_TR_NOOP("The NetISDN Database has\nbeen updated to Version"));
    msg=QString().
	sprintf("%s %d",(const char *)str,NET_VERSION_DATABASE);
